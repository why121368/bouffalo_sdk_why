...
#define PORT        "1883"
#define CLIENTID    "XXXXX"   //设备的名称
#define USERNAME    "XXXXXX" //产品ID
#define PASSWORD    "XXXXXXXX"  //使用onennet的token工具生成密码 工具地址 https://open.iot.10086.cn/doc/v5/fuse/detail/242
#define SUBTOPIC    "$sys/XXXXXXX/???????/thing/property/post/reply"//订阅topic  XXXXXX是产品ID   ??????是设备名称
#define PUBTOPIC    "$sys/XXXXXXX/??????/thing/property/post"      //发布topic
...
int example_mqtt(int argc, const char *argv[])
{
    ...
    while(1) {
        ...
        sprintf(message,"{\"id\":\"123\",\"version\":\"1.0\",\"params\": {\"temp_value\": {\"value\":%.1f}}}",temp);
